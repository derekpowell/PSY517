...
"Tidy" data is data stored in "long" rather than "wide" format.
...
Suppose participant #3 called us up with a confession: they were really distracted the day of the experiment and could hardly pay any attention. Now that we know that, maybe it's best to remove them from our dataset.
...
We can compare two variables with `==`, `!=`, `>`, `<`, `>=`, and `<=`.
...
- After they were done, they weighed the remaining food in the dish to calculate how many grams of food the children ate
...
```{r, include=F}
library(tidyverse)
dejesus<-read_csv("dejesus-example.csv")
```
