...
But we can also turn this logic around: any computation we'd like to do with probabilities can be approximated by working with _samples_ from a probabilitiy distribution and counting up what happens.
...
### Samples can also summarize. We can ...
...
- Classic but unethical experiments testing obedience to authority
...
## Hands-On: Investigating Milgram's priors
...
```{r, echo=T}
prior_predictive <- rbinom(1e4, size=40, prob = milgrams_prior)
```
...
ggplot()+
  ...
  theme_bw() +
  labs(x="Number harmed")
```

## Hands-On: Prior predictive checks
...
- Milgram's prior biases the estimates
- 65% of participants administered the shocks
- But our MAP estimate for $\pi|y$ is .49---the estimate is __biased__.
